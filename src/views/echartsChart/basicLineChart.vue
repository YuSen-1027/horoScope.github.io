...
// JS统计重复元素
function addSameElement<T extends string | number>(arr: T[]): Record<T, number> {
  let count: Record<T, number> = {} as Record<T, number>;
...
// 统计重置好的原数据中有多少个相同的数据
const sameElement = addSameElement(newLineArr.value);
...
// 创造新数组存放最终相同的数据元素
const endSameArr = reactive<any>([]);
// 创造新数组存放最终的时间数据
const newTimArr = reactive<any>([]);
// 创造新数组存放最终的价格数据
const newPrArr = reactive<any>([]);
...
for (let key in sameElement) {
  ...
  let newTime = timestampToDate((splitArr[0] as any) * 1000).slice(5);
  // 转换价格
  let newPrice = (splitArr[1] as any) / 100;
  // 这里是后期 echarts 中 markPoint 所用到的数据格式
  let rose = { name: key, value: sameElement[key], xAxis: newTime, yAxis: (splitArr[1] as any) / 100 };
...
function drawChart(newPrArr: any, newTimArr: any, endSameArr: any) {
